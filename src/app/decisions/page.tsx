...
export default function DecisionsPage() {
  ...
  if (loading) return <div className="p-10 text-gray-400">Loading decisions...</div>
    ...
    <div className="max-w-4xl mx-auto px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Track decisions, reasoning & outcomes</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={saveDecision} className="bg-white border border-gray-200 rounded-xl p-6 mb-6 space-y-4">
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Chosen Option</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Reasoning</label>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button type="button" onClick={resetForm} className="px-4 py-2 text-gray-500 text-sm">Cancel</button>
          </div>
        </form>
      )}
        ...
        {decisions.map(d => {
          ...
          return (
            <div key={d.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              {expanded && (
                <div className="px-5 pb-4 border-t border-gray-100 pt-4 space-y-3 text-sm">
                  {d.context && <div><span className="text-gray-400 text-xs block mb-0.5">Context</span><span className="text-gray-700">{d.context}</span></div>}
                  {d.reasoning && <div><span className="text-gray-400 text-xs block mb-0.5">Reasoning</span><span className="text-gray-700">{d.reasoning}</span></div>}
                  {d.expected_outcome && <div><span className="text-gray-400 text-xs block mb-0.5">Expected Outcome</span><span className="text-gray-700">{d.expected_outcome}</span></div>}
                  {d.actual_outcome && <div><span className="text-gray-400 text-xs block mb-0.5">Actual Outcome</span><span className="text-gray-700">{d.actual_outcome}</span></div>}
                  {d.review_date && !d.actual_outcome && <div className="text-xs text-gray-400">Review scheduled: {d.review_date}</div>}

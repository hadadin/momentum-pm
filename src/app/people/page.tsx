...
export default function PeoplePage() {
  ...
  if (loading) return <div className="p-10 text-gray-400">Loading people...</div>
    ...
    <div className="max-w-6xl mx-auto px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Stakeholders, team members & commitments</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={savePerson} className="bg-white border border-gray-200 rounded-xl p-6 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Working Style</label>
              <input value={workingStyle} onChange={e => setWorkingStyle(e.target.value)} placeholder="e.g. Prefers async, detail-oriented"
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Communication Preference</label>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button type="button" onClick={resetPersonForm} className="px-4 py-2 text-gray-500 text-sm">Cancel</button>
          </div>
        </form>
      )}
          ...
          {people.map(p => (
            <button key={p.id} onClick={() => setSelectedPerson(p.id)}
              className={`w-full text-left px-4 py-3 rounded-xl border transition-colors ${
                selectedPerson === p.id ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-200 hover:border-gray-300'
              }`}>
              <div className="font-medium text-sm text-gray-900">{p.name}</div>
              {p.role && <div className="text-xs text-gray-500">{p.role}</div>}
              {p.team && <div className="text-xs text-gray-400">{p.team}</div>}
            </button>
          ))}
          {people.length === 0 && <p className="text-sm text-gray-400 text-center py-8">No people added yet</p>}
        </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  {selected.role && <p className="text-sm text-gray-500">{selected.role}{selected.team ? ` · ${selected.team}` : ''}</p>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
                  {selected.working_style && (
                    <div className="bg-gray-50 rounded-lg p-3">
                      <div className="text-xs text-gray-400 mb-1">Working Style</div>
                      <div className="text-gray-700">{selected.working_style}</div>
                    </div>
                  )}
                  {selected.communication_preference && (
                    <div className="bg-gray-50 rounded-lg p-3">
                      <div className="text-xs text-gray-400 mb-1">Communication</div>
                      <div className="text-gray-700">{selected.communication_preference}</div>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                {showCommitForm && (
                  <form onSubmit={saveCommitment} className="bg-gray-50 rounded-lg p-4 mb-3 space-y-3">
                    <input value={commitDesc} onChange={e => setCommitDesc(e.target.value)}
                      placeholder="What's the commitment?" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
                ...
                <div className="space-y-2">
                  {personCommitments.map(c => (
                    <div key={c.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${
                      ...
                      <button onClick={() => toggleCommitment(c.id, c.status)}
                        className={`w-4 h-4 rounded border flex-shrink-0 ${c.status === 'fulfilled' ? 'bg-green-500 border-green-500' : 'border-gray-300'}`} />
                      <span className={c.status === 'fulfilled' ? 'line-through text-gray-400' : 'text-gray-700'}>{c.description}</span>
                  ...
                  {personCommitments.length === 0 && <p className="text-xs text-gray-400">No commitments tracked</p>}
                </div>
              </div>
            </div>
          ) : (
